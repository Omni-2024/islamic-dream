    ...
    <div className="fixed bottom-4 right-4 z-50">
      <AnimatePresence>
        ...
        {isOpen && (
          ...
          >
            {/* Chat Header */}
            <div className="bg-RuqyaGreen text-white p-3 flex items-center justify-between">
              <h3 className="font-semibold">{channel.data?.name || "Chat Support"}</h3>
              <div className="flex items-center gap-2">
                <button onClick={() => setIsMinimized(!isMinimized)} className="hover:bg-primary-200 p-1 rounded">
                  <Minimize2 size={18} />
                </button>
                <button onClick={() => setIsOpen(false)} className="hover:bg-primary-200 p-1 rounded">
                  <X size={18} />
                </button>
              </div>
            </div>
                ...
                <div className="w-1/3 border-r border-gray-200 overflow-y-auto">
                  <div className="p-4">
                    ...
                    {allChannels.length > 0 ? (
                      allChannels.map((ch) => (
                        <button key={ch.id} onClick={() => selectChannel(ch)} className={`w-full text-left p-2 rounded mb-2 ${channel?.id === ch.id ? "bg-blue-100" : "hover:bg-gray-100"}`}>
